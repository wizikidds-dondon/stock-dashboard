...
const API = (() => {
  ...
  // 計算對各大法人類股資金流向
  function calcFlowSectors(institutional, days, instType) {
    // 先建立 sector 對照表
    const SECTOR_MAP = {
  ...
  function calcFlowStocks(institutional, instType, limit) {
    ...
    const buy = mapped.filter(r => r.net > 0).sort((a, b) => b.net - a.net).slice(0, limit);
    const sell = mapped.filter(r => r.net < 0).sort((a, b) => a.net - b.net).slice(0, limit);
    return { buy, sell };
  }
  ...
  function calcFlowMatrix(institutional) {
    ...
    const matrix = {}; // matrix[sector][inst_type] = net
    for (const rec of institutional) {
    ...
    const max_abs = Math.max(...Object.values(data).flatMap(v => Object.values(v).map(Math.abs)), 1);
    return { sectors, data, max_abs };
  }
  ...
  return {
    get: async (path) => {
      ...
      const days = parseInt(p.get('days') || '1');
      const instType = p.get('institution') || 'total';
      const limit = parseInt(p.get('limit') || '20');

      if (path.includes('/flow/sectors')) {
        return { sectors: calcFlowSectors(inst, days, instType), date_range: [data.updated_at] };
      }
      if (path.includes('/flow/stocks')) {
        const { buy, sell } = calcFlowStocks(inst, instType, limit);
        return { buy, sell };
      }
      if (path.includes('/flow/matrix')) {
        return calcFlowMatrix(inst);
      }
    ...
    getSectors: async () => { const data = await loadData(); return Object.entries(data.sectors).map(([name, info]) => ({ name, change_pct: info.avg_change_pct, stock_count: info.stocks.length })); },
